...
//                        printf("外部变量偏移: %d\n",T->offset);
                        ...
                        if (T->ptr[1]->kind==ID){ //只定义了一个外部变量名
                            rtn=fillSymbolTable(T->ptr[1]->type_id,newAlias(),LEV,T->ptr[0]->type,'V',T->offset);
                            if (rtn==-1)
                            ...
                            T->num=1;
                              ...
                              semantic_Analysis(T->ptr[1]);     //处理外部变量名序列
                              T->num=T->ptr[1]->num;
                            }
//                        prn_symbol();
                        T->width=(T->type==INT?2:4)* T->num;
                        break;
    case EXT_DEC_LIST:  //处理外部变量名序列结点，对ID类结点在登记到符号表，作为外部变量
                        num=1;
                        T0=T;
                        while (T0->kind==EXT_DEC_LIST) {
                            rtn=fillSymbolTable(T0->ptr[0]->type_id,newAlias(),LEV,T0->type,'V',T0->offset);
                            if (rtn==-1)
                                semantic_error(T0->ptr[0]->pos,T0->ptr[0]->type_id, "变量重复定义");
                            else T0->ptr[0]->place=rtn;
                            T0->ptr[1]->width=T0->width;      //将一个变量的宽度向下传递
                            T0->ptr[1]->offset=T0->offset+T0->width; //这个外部变量的偏移量向下传递
                            T0->ptr[1]->type=T0->type;//将结点类型属性向下传递
                            num++;
                            T0=T0->ptr[1];
                            }
                        rtn=fillSymbolTable(T0->type_id,newAlias(),LEV,T0->type,'V',T0->offset);  //最后一个变量名
                        if (rtn==-1)
                                semantic_error(T->pos,T0->type_id, "变量重复定义");
                        else T0->place=rtn;
                        T->num=num;
                        break;
	case FUNC_DEF:      //填写符号表
  ...
  //                      prn_symbol();       //c在退出一个符合语句前显示的符号表
	...
	case UMINUS:    semantic_Analysis(T->ptr[0]);
                    ...
                    break;
    case FUNC_CALL: //根据T->type_id查出函数的定义，向下传递相关属性
                    rtn=searchSymbolTable(T->type_id);
                        if (rtn==-1)
                            semantic_error(T->pos,T->type_id, "变量未定义");
                        break;
                    semantic_Analysis(T->ptr[0]);//处理所以实参表达式
                    break;
	case ARGS:      //处理所以实参表达式，
                    break;
         }
